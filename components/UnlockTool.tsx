...
import { Upload, LockOpen, AlertCircle, Download, Key, Settings, Loader2, ChevronDown, Cpu, StopCircle, RefreshCw, FileText, ShieldCheck } from 'lucide-react';
import PdfWorker from './pdfWorker?worker';

// @ts-ignore
import QPDF from 'qpdf-wasm-esm-embedded';

// ---------- Massive password list from the second version ----------
...
export default function UnlockTool() {
  ...
  // Worker‑based dictionary attack (from second version)
  const runDictionaryAttack = (passwordsToTry: string[], pdfBytes: Uint8Array, totalPasswords: number): Promise<void> => {
      ...
      for (let i = 0; i < numCores; i++) {
        ...
        worker.onmessage = (msg) => {
          ...
          if (type === 'fatal_error') {
            ...
            setCurrentTry(wTry);
            attempts++;
            setProgress(Math.min(100, Math.round((attempts / totalPasswords) * 100)));
          } else if (type === 'done') {
            activeWorkers--;
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const currentTriedSet = new Set<string>(autoTryPasswords);
      setTriedPasswords(currentTriedSet);

      // 1. Dictionary attack on common + filename passwords
      await runDictionaryAttack(autoTryPasswords, pdfBytes, autoTryPasswords.length);
      if (status === 'unlocked' || stopBruteForceRef.current) return;
      ...
      try {
        ...
        if (response.ok && data.success && data.passwords) {
          ...
          if (dbPasswords.length > 0) {
            await runDictionaryAttack(dbPasswords, pdfBytes, dbPasswords.length);
            if (status === 'unlocked' || stopBruteForceRef.current) return;
          }
        }
      } catch (apiError) {
      ...
      // 3. If still locked and not AES‑256, start numeric brute force (from first version)
      if (!isAes256 && status !== 'unlocked' && !stopBruteForceRef.current) {
        setStatus('number_bruteforce');
        const numCores = navigator.hardwareConcurrency || 4;

        for (let length = 1; length <= 9; length++) {
          if (stopBruteForceRef.current || status === 'unlocked') break;
      ...
      // 4. If still not unlocked, show password request
    ...
    <div className="min-h-screen bg-[#FDF8F6] font-sans text-slate-900 selection:bg-rose-100 selection:text-rose-700 pb-10 md:pb-20 relative">
      ...
      <div className="relative z-10 w-full max-w-4xl mx-auto px-3 sm:px-6 py-4 md:py-12">
        ...
        <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl shadow-rose-500/10 border border-rose-50 relative overflow-hidden transition-all duration-500">
          ...
          {/* number_bruteforce UI from first version */}

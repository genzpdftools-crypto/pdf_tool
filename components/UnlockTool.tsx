...
const MAX_SMART_ATTEMPTS = 2000000; 
...
export default function UnlockTool() {
  ...
  // 🔥 COMPLETELY FIXED SMART RECOVERY (ELIMINATION METHOD) 🔥
  const handleSmartUnlock = async () => {
    ...
    // Helper: Combinations for Scattered Chars
    ...
    // Helper: Permutations for Scattered Chars
    const getUniquePermutations = (str: string): string[] => {
      if (str.length <= 1) return [str];
      if (str.length > 5) return Array.from(new Set([str, str.split('').reverse().join('')]));
      const perms = new Set<string>();
      const permute = (arr: string[], m: string[] = []) => {
        if (arr.length === 0) perms.add(m.join(''));
        else {
    ...
    const reqA = exactAlphabets !== '' ? parseInt(exactAlphabets) : -1;
    const reqN = exactNumbers !== '' ? parseInt(exactNumbers) : -1;
    const reqS = exactSymbols !== '' ? parseInt(exactSymbols) : -1;
    ...
    for (const phase of phases) {
      ...
      for (let len = lenMin; len <= lenMax; len++) {
        ...
        // 1. ELIMINATION: Make base template (Fix first/last char)
        let baseTemplate = Array(len).fill(null);
        let conflict = false;

        if (firstChar) baseTemplate[0] = firstChar;
        if (lastChar) {
           if (baseTemplate[len - 1] !== null && baseTemplate[len - 1].toLowerCase() !== lastChar.toLowerCase()) {
               conflict = true; // Agar length 1 hai aur first/last match nahi karta to conflict
           }
           baseTemplate[len - 1] = lastChar;
        }
        if (conflict) continue;

        // DFS Engine (Fills only empty spots and prunes invalid counts immediately)
        const runDfs = async (template: (string | null)[]) => {
            const emptyIndices = template.map((c, i) => c === null ? i : -1).filter(i => i !== -1);
            
            // Check Base Counts
            let baseA = 0, baseN = 0, baseS = 0;
            for (const c of template) {
                if (c !== null) {
                    const cCode = c.charCodeAt(0);
                    if ((cCode >= 65 && cCode <= 90) || (cCode >= 97 && cCode <= 122)) baseA++;
                    else if (cCode >= 48 && cCode <= 57) baseN++;
                    else baseS++;
                }
            }

            // Master Pruning (Faltu template skip karo)
            if (reqA !== -1 && (baseA > reqA || baseA + emptyIndices.length < reqA)) return;
            if (reqN !== -1 && (baseN > reqN || baseN + emptyIndices.length < reqN)) return;
            if (reqS !== -1 && (baseS > reqS || baseS + emptyIndices.length < reqS)) return;

            const stack = [{ t: [...template], eIdx: 0, a: baseA, n: baseN, s: baseS }];

            while (stack.length > 0) {
                if (unlocked || stopBruteForceRef.current || attempts >= MAX_SMART_ATTEMPTS) break;
                const { t, eIdx, a, n, s } = stack.pop()!;

                if (eIdx === emptyIndices.length) {
                    if (reqA !== -1 && a !== reqA) continue;
                    if (reqN !== -1 && n !== reqN) continue;
                    if (reqS !== -1 && s !== reqS) continue;

                    const pwd = t.join('');
                    if (triedPasswords.has(pwd)) continue;
                    triedPasswords.add(pwd);

                    attempts++;

                    // WASM Memory crash solver
                    if (useWasm && attempts % 1000 === 0 && attempts > 0) {
                        try {
                            if(qpdfInstance) {
                               try { qpdfInstance.FS.unlink('smart_input.pdf'); qpdfInstance.FS.unlink('smart_output.pdf'); } catch(e){}
                            }
                            qpdfInstance = await QPDF();
                            qpdfInstance.FS.writeFile('smart_input.pdf', pdfBytes);
                        } catch(e) {}
                    ...
                    const timeLimit = useWasm ? 30 : 50;
                    if (Date.now() - lastYieldTime > timeLimit) {
                        setProgress(Math.min(100, Math.round((attempts / 100000) * 100)));
                        setCurrentTry(pwd);
                        await new Promise(resolve => setTimeout(resolve, 0));
                    ...
                    if (useWasm && qpdfInstance) {
                        try {
                            ...
                            qpdfInstance.callMain(['--password=' + pwd, '--decrypt', 'smart_input.pdf', 'smart_output.pdf']);
                ...
                const targetIdx = emptyIndices[eIdx];
                const remSpots = emptyIndices.length - eIdx - 1;

                for (let i = pool.length - 1; i >= 0; i--) {
                    const char = pool[i];
                    let isA = 0, isN = 0, isS = 0;
                    const cCode = char.charCodeAt(0);
                    if ((cCode >= 65 && cCode <= 90) || (cCode >= 97 && cCode <= 122)) isA = 1;
                    else if (cCode >= 48 && cCode <= 57) isN = 1;
                    else isS = 1;

                    // Pruning on the fly!
                    if (reqA !== -1) {
                        if (a + isA > reqA) continue;
                        if (a + isA + remSpots < reqA) continue;
                    }
                    if (reqN !== -1) {
                        if (n + isN > reqN) continue;
                        if (n + isN + remSpots < reqN) continue;
                    }
                    if (reqS !== -1) {
                        if (s + isS > reqS) continue;
                        if (s + isS + remSpots < reqS) continue;
                    }

                    const nextT = [...t];
                    nextT[targetIdx] = char;
                    stack.push({ t: nextT, eIdx: eIdx + 1, a: a + isA, n: n + isN, s: s + isS });
                }
            }
        }; 
        ...
        if (phase === 1) {
            if (!middleHint) {
                await runDfs(baseTemplate);
            } else {
                // 2. ELIMINATION: Contiguous Substring (Ek sath)
                for (let start = 0; start <= len - middleHint.length; start++) {
                    if (unlocked) break;
                    let matches = true;
                    const t = [...baseTemplate];
                    for (let i = 0; i < middleHint.length; i++) {
                        if (t[start + i] !== null && t[start + i]?.toLowerCase() !== middleHint[i].toLowerCase()) {
                            matches = false;
                            break;
                        }
                        t[start + i] = middleHint[i];
                    }
                    if (matches) await runDfs(t); // Only runs if perfectly fits!
                }
            }
        } else if (phase === 2 && middleHint) {
            // 3. ELIMINATION: Scattered Substring (Alag alag)
            const emptyIndices = baseTemplate.map((c, i) => c === null ? i : -1).filter(i => i !== -1);
            if (emptyIndices.length >= middleHint.length) {
                const indexCombos = getCombinations(emptyIndices, middleHint.length);
                const perms = getUniquePermutations(middleHint);

                for (const indices of indexCombos) {
                    if (unlocked) break;
                    for (const perm of perms) {
                        ...
                        // Skip if they are contiguous (already checked in Phase 1)
                        const dummyStr = t.map(c => c === null ? '_' : c).join('');
                        if (!dummyStr.toLowerCase().includes(middleHint.toLowerCase())) {
                            await runDfs(t);
                        }
                    }
                }
    ...
    if (!unlocked && !stopBruteForceRef.current) {
      setStatus('needs_password');
      setErrorMessage(`Smart Cracking Failed. Checked exactly ${attempts} valid combinations.`);
    }
  };

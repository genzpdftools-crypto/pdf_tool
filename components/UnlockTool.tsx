...
export default function UnlockTool() {
  ...
  // 🔥 THIS IS YOUR EXACT PERFECT AUTOMATIC CODE 🔥
  ...
  // 🔥 THIS IS THE FIXED SMART RECOVERY ENGINE 🔥
  const handleSmartUnlock = async () => {
    ...
    const hasScatteredChars = (pwd: string, hint: string) => {
      let temp = pwd.toLowerCase();
      for (const char of hint.toLowerCase()) {
        ...
        for (const variant of hintVariants) {
          ...
          for (const mIdx of startIndices) {
            ...
            if (phase === 1 && variant && mIdx !== null) {
                if (firstChar && mIdx === 0 && variant[0].toLowerCase() !== firstChar.toLowerCase()) continue;
                if (lastChar && mIdx + variant.length === len && variant[variant.length - 1].toLowerCase() !== lastChar.toLowerCase()) continue;
            }
            ...
            while (stack.length > 0) {
              ...
              if (depth === len) {
                if (middleHint) {
                  if (phase === 1) {
                    if (!str.toLowerCase().includes(variant.toLowerCase())) continue;
                  } else if (phase === 2) {
                    if (!hasScatteredChars(str, middleHint) || str.toLowerCase().includes(middleHint.toLowerCase())) continue;
                  }
                }
                ...
                if (Date.now() - lastYieldTime > timeLimit) {
                  setProgress(Math.min(100, Math.round((attempts / 200000) * 100)));
                  setCurrentTry(`${str}`); 
                  await new Promise(resolve => setTimeout(resolve, 0)); 
    ...
    if (!unlocked && !stopBruteForceRef.current) {
      setStatus('needs_password');
      setErrorMessage(`Smart Cracking Failed. Checked ${attempts} combinations based on exact hints.`);
    }
  };

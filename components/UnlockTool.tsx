...
import { Upload, LockOpen, AlertCircle, Download, Key, Settings, Loader2, ChevronDown, ChevronUp, Cpu, RefreshCw, FileText, ShieldCheck } from 'lucide-react';
...
const MAX_SMART_ATTEMPTS = 500000; 
...
export default function UnlockTool() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // 🔥 AUTO PROGRESS BAR FIX 🔥
      const totalLocalPass = autoTryPasswords.length;
      let localCount = 0;

      for (const pwd of autoTryPasswords) {
        localCount++;
        
        if (localCount % 10 === 0 || localCount === 1) {
          setCurrentTry(pwd);
          setProgress(Math.round((localCount / totalLocalPass) * 100));
  ...
  const handleManualUnlock = async () => {
    ...
    try {
      ...
      try {
        ...
        // Bina condition check kiye ALWAYS WASM try karega error aane par
        setIsAes256(true); 
        try {
           const bytes = await unlockWithWasm(manualPassword, pdfBytes);
           setUnlockedPdfBytes(bytes);
  ...
  // 🔥 COMPLETELY FIXED SMART RECOVERY (ELIMINATION METHOD + FRESH INSTANCE) 🔥
            ...
            while (stack.length > 0) {
                ...
                if (eIdx === emptyIndices.length) {
                    ...
                    if (Date.now() - lastYieldTime > timeLimit) {
                        setProgress(Math.min(100, Math.round((attempts / 10000) * 100)));
                        setCurrentTry(pwd);
                        await new Promise(resolve => setTimeout(resolve, 0));
                        lastYieldTime = Date.now();
                    }

                    // 🔥 FRESH INSTANCE ENGINE CHECK 🔥
                    if (useWasm) {
                        try {

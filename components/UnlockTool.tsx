...
export default function UnlockTool() {
  ...
  // ----- IMPROVED SMART UNLOCK WITH EARLY PRUNING AND SAFETY LIMITS -----
  const handleSmartUnlock = async () => {
    if (!file) return;

    // 1. UI KO RENDER HONE DO (Breathing Room)
    setStatus('smart_cracking');
    setErrorMessage('');
    setProgress(0);
    stopBruteForceRef.current = false;

    // React ko loader dikhane ke liye 100ms ki saans do, isse turant freeze nahi hoga
    await new Promise(resolve => setTimeout(resolve, 100));

    const pool = getCharPool();
    let combinations: string[] = [];
    
    // 2. EMERGENCY BRAKE (Taaki kabhi "Page Unresponsive" na aaye)
    let iterations = 0;
    const MAX_ITERATIONS = 5000000; // 5 Million limit
    ...
    try {
      const generate = (currentStr: string, targetLen: number) => {
        ...
        if (iterations > MAX_ITERATIONS) {
           // Agar search bahut badi ho gayi toh error fek do
           throw new Error('TOO_MANY_COMBINATIONS');
        }

        // 3. EARLY PRUNING (Super Speed Logic)
        ...
        const remainingSpace = targetLen - currentStr.length;

        // Agar numbers/alphabets zarurat se zyada ho gaye, YA unke aane ki jagah hi nahi bachi, toh rasta cut karo
        if (exactAlphabets !== '') {
            const req = parseInt(exactAlphabets);
            if (alphaCount > req) return; 
            if (alphaCount + remainingSpace < req) return; 
        }
        if (exactNumbers !== '') {
            const req = parseInt(exactNumbers);
            if (numCount > req) return;
            if (numCount + remainingSpace < req) return;
        }
        if (exactSymbols !== '') {
            const req = parseInt(exactSymbols);
            if (symCount > req) return;
            if (symCount + remainingSpace < req) return;
        }

        // Agar word pura ban gaya hai
        ...
        // Aage badho combinations ke liye
      ...
      for (let len = lenMin; len <= lenMax; len++) {
       ...
       if (err.message === 'TOO_MANY_COMBINATIONS') {
          setStatus('needs_password');
          setErrorMessage('⚠️ Hints bahut aam hain (Search space huge hai). Kripya Length Range kam karein ya aur Hints dalein taaki device hang na ho.');
          return;
       }
    }
    ...
    if (combinations.length === 0) {
      setStatus('needs_password');
      setErrorMessage('Aapke hints se ek bhi password match nahi bana. Kripya rules thode asaan karein.');
      return;
    }

    // Yahan se aage tumhara purana "CASE 1: WASM" aur "CASE 2: Worker" wala logic start hoga
    const arrayBuffer = await file.arrayBuffer();
    const pdfBytes = new Uint8Array(arrayBuffer);

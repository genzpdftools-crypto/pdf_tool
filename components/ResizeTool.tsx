...
interface ResizeToolProps {
  initialTargetKb?: string | null; // dynamic target from URL, e.g. 100kb
}
...
export const ResizeTool: React.FC<ResizeToolProps> = ({ initialTargetKb }) => {
  ...
  const getPageTitle = () => {
    if (initialTargetKb) {
      return `Resize Image to ${initialTargetKb}KB Online Free | Genz PDF`;
    }
    return "Resize Image Online - Change Dimensions (JPG, PNG, WebP) | Genz PDF";
  };

  const getPageDescription = () => {
    if (initialTargetKb) {
      return `Instantly compress and resize your image to exact ${initialTargetKb}KB online for free. Secure, fast, and 100% client-side processing.`;
    }
  ...
  return (
    <>
      {/* ✅ SEO Component – dynamic based on initialTargetKb */}
      <SEO
        title={getPageTitle()}
        description={getPageDescription()}
        url={getCanonicalUrl()}
        type="SoftwareApplication"
        keywords={initialTargetKb 
          ? `resize image to ${initialTargetKb}kb, compress image to ${initialTargetKb}kb, ${initialTargetKb}kb photo resizer`
          : "resize image, change image dimensions, resize photo online free, jpg resize, png resize, webp resize"}
      />
